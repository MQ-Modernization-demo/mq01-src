...
listener runs the pipeline using a Tekton **trigger**.  A trigger has two
components: a `triggerbinding` that takes data from the GitHub event and passes
 it to a `triggertemplate` that specifies the pipeline to run.
...
processor of that change. The design is slightly more complex than directly
linking GitHub to a pipeline, but as we'll see, this design is much more
flexible. This [short
...
interceptors](https://tekton.dev/docs/triggers/), though the key point to note
is that our event listener will intercept GitHub events (`X-GitHub-Event`) that
refer to a `push` to a repository `$GITORG/mq01-src`. These events will result
in `mq-trigger-binding` and `mq-trigger-template` being used to run the pipeline
with the required inputs, as well see later.

Let's tailor the event listener YAML for our organization.

```bash
envsubst < mq-event-listener.yaml > eventfile.tmp && mv eventfile.tmp mq-event-listener.yaml
```

You can re-issue the following command to verify that `$GITORG/mq01-src` value
...
Examine the `params.filter` value to confirm the substitution.

We now apply the following YAMLs to the cluster to create the event listener,
its route and service:
...
Notice that the event listener service `el-mq-event-listener` is created
automatically by the event listener, and referred to by the event listener
route. Now that we've got the **event listener** box in our diagram fully
deployed, let's configure and install the event trigger.

---

## Creating the trigger

The Tekton trigger comprises a **trigger binding** that gets the necessary data
from the webhook event, and a **trigger template** that uses this data to call
the pipeline.

Let's examine, customize and deploy the trigger binding and template that will
run the pipeline to build `mq01`.
...
Issue the following command to examine the trigger template:
...
We now apply the following YAMLs to the cluster to create the trigger binding
and template:
...
Issue the following commands to verify the trigger binding and trigger template
have been created:
...
We now configure the GitHub web hook
...
sends it to the route whenever the queue manager source repository changes.
